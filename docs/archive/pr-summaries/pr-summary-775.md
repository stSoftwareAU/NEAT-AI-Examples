...
into a new `common/feed_forward_network.ts` that both examples import. The synthetic-synapse
demo's own bookkeeping (`synthetic` synapse flag, `originalSynapseKeys`) stays caller-side, and
...
Backend/library change with no web interface — no screenshot applies. The evidence is the
regression test below, which fails against the unfixed `synthetic_synapse` copy and passes after
the extraction:
...
- `activate` evaluates each supported squash, including the numerically stable negative-`z`
  logistic branch.
...
- `heldOutScore` (and through it the private `mseAgainst`) scores a perfect fit at zero, penalises
  a mismatch with the expected magnitude and sign, and returns zero for an empty dataset.
...
- Added `forward - hidden→hidden cascade reads the upstream hidden activation` — the regression
  test for this issue at the site that had the bug. No existing test was removed or weakened.

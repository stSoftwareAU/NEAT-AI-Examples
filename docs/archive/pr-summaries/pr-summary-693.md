...
The `mnist_classification/` example named the private `stSoftwareAU/GRQ` repository and pointed
readers at its internal scripts (`sampler.sh`, `worker/shared/squash_random.sh`,
`.sampler/loop-N.json`) across module docs, code comments, exported identifiers, console output, and
test names. None of those pointers resolve for a public reader.

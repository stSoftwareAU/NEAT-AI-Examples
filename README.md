...
[![Quality Check](https://github.com/stSoftwareAU/NEAT-AI-Examples/actions/workflows/quality.yml/badge.svg?event=pull_request)](https://github.com/stSoftwareAU/NEAT-AI-Examples/actions/workflows/quality.yml)
...
| [📍 TSP Constructive](tsp_constructive/README.md)                     | 🗺️ routing    | Evolve a NEAT controller that builds a TSPLIB tour one city at a time by picking from the K-nearest unvisited cities; renders the champion tour SVG and the milestone-stats chart.                                                                        | `./tsp_constructive/run.sh`     |
| [🧭 TSP 2-Opt](tsp_two_opt/README.md)                                 | 🗺️ routing    | Evolve a learned 2-opt local-search operator that improves a nearest-neighbour seed tour within a fixed swap budget; renders the seed vs improved tour side-by-side.                                                                                      | `./tsp_two_opt/run.sh`          |
...
| [🔬 Discovery at Scale](discovery_at_scale/README.md)                 | 🛠 technique   | Evolve a 9-neuron minimal seed via `evolveDir` until it reproduces the labels of a moderately-large hand-crafted reference creature; the milestone summary's seed-vs-final topology bars show how much hidden structure NEAT-AI added on top of the seed. | `./discovery_at_scale/run.sh`   |
...
| [🔢 MNIST](mnist_classification/README.md)                            | 📊 supervised | Evolve a 784 → 10 logistic classifier on the full 28×28 MNIST training set (60 000 images) and render an animated grid of predictions.                                                                                                                    | `./mnist_classification/run.sh` |
...
| ---------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Minimal-seed evolution at scale    | [🔬 Discovery at Scale](discovery_at_scale/README.md) | Evolve a `new Creature(6, 3)` minimal seed until it reproduces the labels of a moderately-large hand-crafted reference creature; the milestone summary's seed-vs-final topology bars show how much hidden structure NEAT-AI added on top of the minimal seed. | ![Discovery at Scale — milestone summary with seed-vs-final topology bars showing hidden neurons and synapses added on top of the minimal seed](docs/screenshots/discovery_at_scale/evolution_summary.svg) |
...
flowchart TD
    ...
    MNIST["🔢 MNIST<br/>Classify handwritten digits<br/>from the full 28×28 input"]
